...
public class Dijkstra {
    ...
    public ArrayList<int[]> findGhost(int[] currentPosition, ArrayList<int[]> openSetPosition, int[][] map, int[] ghostPosition) {
        ArrayList<int[]> path = new ArrayList<int[]>();
        ArrayList<int[]> openSet = new ArrayList<int[]>();
        ArrayList<Node> closeSet = new ArrayList<Node>();
                ...
                for (int j = 0; j < 4; j++) {
                    ...
                    for (int k = 0; k < openSet.size(); k++) {
                        if (openSet.get(k)[0] == newPosition[0] && openSet.get(k)[1] == newPosition[1]) {
                            ...
                            if (node.position[0] == ghostPosition[0] && node.position[1] == ghostPosition[1]) {
                                i = currentLayer.size();
                                j = 4;
                                ghostFound = true;
                            }
